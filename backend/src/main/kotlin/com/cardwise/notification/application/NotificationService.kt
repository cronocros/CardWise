...
class NotificationService(
    ...
    override fun getSettings(accountId: UUID): NotificationSettingsResponse {
        return notificationSettingRepository.findOrCreateDefault(accountId).toResponse()
    }

    @Transactional
    override fun patchSettings(accountId: UUID, request: NotificationSettingsPatchRequest): NotificationSettingsResponse {
    ...
    override fun listNotifications(accountId: UUID, type: String?, limit: Int): List<NotificationItemResponse> {
    ...
    override fun getUnreadCount(accountId: UUID): NotificationUnreadCountResponse {
        return NotificationUnreadCountResponse(unreadCount = notificationRepository.countUnread(accountId))
    }

    @Transactional
    override fun markRead(accountId: UUID, notificationId: Long): NotificationItemResponse {
    ...
    @Transactional
    override fun markAllRead(accountId: UUID): NotificationUnreadCountResponse {
        notificationRepository.markAllRead(accountId)
        return getUnreadCount(accountId)
    }

    @Transactional
    override fun createNotification(command: NotificationInsertCommand): NotificationItemResponse {
        return notificationRepository.insert(command).let(::toItemResponse)
    }

    @Transactional
    override fun createNotificationIfAccountExists(command: NotificationInsertCommand): NotificationItemResponse? {
    ...
    @Transactional
    override fun createNotificationForEmail(
        email: String,
    ...
    @Transactional
    override fun markReferenceRead(accountId: UUID, referenceTable: String, referenceId: Long) {
