...
@Component
class BenefitReadAdapter(
    private val jdbcTemplate: NamedParameterJdbcTemplate,
) : BenefitReadPort {

    override fun findCategories(limit: Int): List<BenefitCategoryRow> {
        val sql = """
            select
    ...
    override fun findCardBenefitHeader(accountId: UUID, cardId: Long): CardBenefitHeaderRow? {
        val sql = """
            with latest_performance as (
                select distinct on (up.user_card_id)
                    up.user_card_id,
    ...
    override fun findCardBenefits(accountId: UUID, cardId: Long): List<BenefitSearchRow> {
        val sql = """
            with latest_performance as (
                select distinct on (up.user_card_id)
                    up.user_card_id,
