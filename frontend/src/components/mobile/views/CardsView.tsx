...
interface CardsViewProps {
  cards: Card[];
  router: any; 
  onToggleMain: (cardId: string) => void;
  onTogglePin: (cardId: string) => void;
}

export function CardsView({ cards, router, onToggleMain, onTogglePin }: CardsViewProps) {
  ...
  return (
    <div className="animate-spring space-y-4 pt-2">
      <div className="space-y-4">
        {sortedCards.map(card => (
          <div key={card.id} onClick={() => router.push('/mobile/card-detail')} className="cursor-pointer">
            <CreditCardComponent card={card} onToggleMain={onToggleMain} onTogglePin={onTogglePin} />
          </div>
        ))}
      </div>
      
      <button 
        onClick={() => router.push('/mobile/add-card')}
