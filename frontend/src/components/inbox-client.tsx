...
export function InboxClient({ initialActions, initialCount }: InboxClientProps) {
  ...
  async function load(nextStatus = status, nextPriority = priority) {
    ...
    try {
      ...
      if (!actionsRes.ok) {
        throw new Error(await actionsRes.text());
      }
      if (!countRes.ok) {
        throw new Error(await countRes.text());
      }
  ...
  async function resolve(action: PendingAction) {
    ...
    try {
      const payload: Record<string, unknown> = {
        resolution: {
          action:
            resolveActionMap[action.actionType] ??
            "KEEP_AS_IS",
        },
      };
      ...
      const response = await fetch(
        `/api/pending-actions/${action.pendingActionId}/resolve`,
        {
          method: "PATCH",
  ...
  return (
    <div className="grid gap-5">
      <Panel
        title="Inbox control"
        subtitle="Filter unresolved items and resolve or dismiss them without leaving the page."
      >
      ...
      <Panel title="Pending actions" subtitle="Resolve or dismiss actions in the order that matters.">
              ...
              >
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex shrink-0 flex-col gap-2 lg:w-64">
                    {(item.actionType === "FX_CORRECTION_NEEDED" ||
                      item.actionType === "BILLING_DISCOUNT_FOUND") && (
                      <TextField
                    ...
                    <div className="flex gap-2">
                      <ActionButton
                        ...
                        disabled={resolvingId === item.pendingActionId}
                        className="flex-1"
                      >
                        Resolve
                      </ActionButton>
                      <ActionButton
                        kind="secondary"
                        onClick={() => dismiss(item.pendingActionId)}
                        disabled={resolvingId === item.pendingActionId}
                        className="flex-1"
                      >
                        Dismiss
