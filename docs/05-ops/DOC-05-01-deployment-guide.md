...
| 항목 | 로컬 / 스테이징 / 프로덕션 (Cloud Only) | 비고 |
| --- | --- | --- |
| 프론트엔드 | **Vercel** (cardwise.app / Preview URL) | Next.js |
| 백엔드 | Cloud Run / **Vercel** (Serverless) | Spring Boot / BFF |
| DB | **Supabase Remote (Cloud)** | 로컬 DB 미사용 |
| Cache | **Upstash (Remote Redis-api)** | 로컬 Redis 미사용 |
| Claude API | Anthropic API (Cloud) | 동일 |
  ...
  backend:
    ...
    services:
      postgres:
        ...
        ports: ["5432:5432"]
      redis:
        # CI/CD용 임시 Redis (Upstash 모사가 불가능할 경우만 사용)
...
> 로컬 개발 시에도 반드시 원격 환경변수를 사용합니다: `SUPABASE_URL`, `UPSTASH_REDIS_REST_URL` 등
...
| REDIS_MODE | 전체 | `upstash` (Remote 전용) |
