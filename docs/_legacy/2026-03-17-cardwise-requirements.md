...
| F3 | 가계부 (이메일 파싱) | 이메일 결제 내역 자동 파싱 -> Draft -> 사용자 확인 | R3 |
| F4 | 연간/월간 실적 관리 | 자동 집계, 실적 구간 충족 여부 판정 | R2 |
| F5 | 혜택 검색 | 가맹점/카테고리 검색 -> 최적 카드 추천 | R4 |
| F6 | 바우처 관리 | 사용/미사용 체크, 잔여 횟수 추적, 만료 알림 | R2 |
| F7 | 알림 | 바우처 만료 알림, 실적 리마인더 | R2 |
| F8 | 대시보드 | 월간 종합, 카테고리별/카드별/태그별 통계 | R5 |
