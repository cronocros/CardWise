...
| F2 가계부 수동입력 | 80% | 기본 저장 + 수정/삭제 완성, card_benefit 매칭 미완 |
| F3 인박스 | 95% | 거의 완성 |
| F4 실적 관리 | 95% | 거의 완성, 구간 변경 알림/모달 연동 완료 |
| F5 혜택 검색 | 95% | 거의 완성 |
| F6 바우처 관리 | 95% | 만료 알림 스케줄러 완성 |
| F7 알림 | 95% | 그룹 기능 연동 완성 |
| F8 대시보드 | 100% | 사용자 소비 대시보드 및 백엔드 API 구현 완료 |
| F12 그룹 가계부 | 95% | 거의 완성 |
...
5. **F8 그룹 대시보드 고도화** — 그룹별 카테고리 심화 분석 및 멤버 비교 구현
